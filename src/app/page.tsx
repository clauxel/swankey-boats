...
export default function HomePage() {
 ...
 <section className="stories"><div className="section-heading"><p className="overline">EXPLORE OUR WORLD</p><h2>The Swankey experience.</h2><span className="heading-rule" /></div><div className="story-grid">{stories.map(s => <Link className="story-card" href={s.href} key={s.title}><div className={`story-image${s.tag === "TECHNOLOGY" ? " system-story-image" : ""}`}><Image src={s.image} width={600} height={400} alt="" /></div><div className="story-copy"><p className="story-tag">{s.tag}</p><h3>{s.title}</h3><p>{s.text}</p><span className="story-link">Explore <span aria-hidden="true">↗</span></span></div></Link>)}</div></section>
 </main>;
}
