...
export function HomeHero() {
  ...
  return (
    <section className="home-hero" aria-label="Swankey on the water">
      <video ref={video} className="hero-video" src="/media/swankey-water-film.mp4?v=3" poster="/media/film-poster.jpg" autoPlay muted={muted} loop playsInline preload="metadata" onPlay={() => setPlaying(true)} onPause={() => setPlaying(false)} aria-label="Boating film across inland lakes" />
      ...
      <dialog ref={dialog} className="film-dialog" aria-label="On the water film" onCancel={(event) => { event.preventDefault(); closeFilm(); }} onClick={(event) => {if(event.target === event.currentTarget) closeFilm();}}>
        <div className="film-shell"><button className="film-close" onClick={closeFilm}>Close film ×</button><video ref={film} src="/media/swankey-water-film.mp4?v=3" poster="/media/film-poster.jpg" controls playsInline preload="none" aria-label="On the water — boating film" /></div>
      </dialog>
    </section>
  );
}
